...
Sometimes, when using forms libraries like [Formik](http://github.com/jaredpalmer/formik) or [React Final Form](https://github.com/final-form/react-final-form), it need to **provide same name-prefix, and other props** to the few form Fields (or ErrorMessage, Custom Component, etc.). So, here for you: 
```javascript
...
### Providing props to the form Fields
...
<FieldSet required={true}>
  <Field name="email"/>
  <Field name="social.facebook" component={CustomInputComponent}/>
  <CustomFieldComponent name="social.twitter"/>
</FieldSet>
```
You can provide in props whatever you want, include functions or object, but check [Perfomance notes](#perfomance)
...
# Perfomance
* FieldSet provides its context for all child components wrapped withFieldSet, so **avoid passing inline-functions and objects to FieldSet props** if possible - this will cause each of connected child components to be re-rendered each time with Formik, which can affect performance. 
